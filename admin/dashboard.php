...
try {
  $s = $pdo->query("SELECT estado, COUNT(*) c FROM expedientes GROUP BY estado");
...
try{
  $q = $pdo->query("SELECT rol, COUNT(*) c FROM usuarios GROUP BY rol");
...
<main class="main">
  <div class="container">
      ...
      <div class="content">
        <div>
          ...
          <div class="actions">
            ...
            <a class="btn soft" href="asignar-expediente.php">Asignar expediente</a>
          </div>
        </div>
    ...
    <section class="grid grid3">
      ...
      <div class="card"><h3><?php echo (int)$uAsesor; ?></h3><p class="muted">Asesores</p></div>
        ...
        <table class="table">
          ...
          <tbody>
            <?php
              ...
              if (!$rows) {
                echo '<tr><td colspan="6">No hay expedientes todavía.</td></tr>';
              } else {
                foreach ($rows as $r) {
                  $st = $r['estado'] ?? '';
                  $cls = estadoClass($st);
    ...
    <?php elseif ($tab === 'usuarios'): ?>
      <section class="card">
        ...
        <table class="table">
          ...
          <tbody>
            <?php
              $q = $pdo->query("SELECT id, nombre, email, rol, created_at FROM usuarios ORDER BY created_at DESC");
              $rows = $q->fetchAll();
